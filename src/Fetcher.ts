...
class Fetcher {
  ...
  async report(url : string) : Promise<void> {
    ...
    const title = await page.evaluate(() => document.querySelector('#right-part > div.issueheader > div:nth-child(2) > h1 > span').textContent);
  
    const contentDesc = await page.evaluate(() => document.querySelector('ezt-comments').getAttribute('description-list'))
    const content = JSON.parse(contentDesc)[0]['content']
